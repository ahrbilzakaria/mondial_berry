...
export const LanguageButton = () => {
  const [value, setValue] = useState<string>(""); // default language
  const router = useRouter();
  const pathname = usePathname();

  useEffect(() => {
    // Check if the cookie exists and set the initial value
    ...
    if (cookieValue) {
      ...
      setValue(decodeURIComponent(lang));
    } else {
      // If no cookie, default to 'fr'
      setValue("fr");
    }
    const segments = pathname.split("/");
    const lg = segments[1];
    if (lg === "es" || lg === "fr" || lg === "en") {
      setValue(lg);
    } else {
      setValue("fr"); // Default to 'fr' if the path doesn't match
    }
  }, []);
  ...
  const handleChange = (val: string) => {
    ...
    // Extract path segments and replace the locale
    const segments = pathname.split("/");
    segments[1] = val; // replace the locale segment (assuming /fr, /es, etc.)
  ...
  return (
    <Select value={value} onValueChange={handleChange}>
      <SelectTrigger className=" hover:bg-white/50 hover:text-primary shadow-none border-1 border-white text-white rounded-full  md:px-4 md:py-6   md:text-[1.125rem] text-md font-medium cursor-pointer">
